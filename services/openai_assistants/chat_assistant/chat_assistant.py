...
class ChatAssistant(Assistant):
    ...
    def _extract_with_regex(
        ...
        try:
            ...
            message_for_human = (
                human_match.group(1) if human_match else ai_message_content
            )

            # Extract message_for_system
            system_match = re.search(
                r'"message_for_system"\s*:\s*(\{.*?\})', sanitized_content, re.DOTALL
            )
    ...
    def _default_parsing(
        ...
        """
        Robustly parses AI response content, extracting `message_for_human`, creating a dict instance for `message_for_system AND pass over the error if it occurs.
        """
        message_for_human, message_for_system, error = self._parse_with_fallbacks(
            ai_message_content
        )
